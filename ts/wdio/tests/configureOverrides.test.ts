...
describe('WebdriverIO: Axe Watcher with Global configurations overrides', () => {
  ...
  before(() => {
    ...
    createAndSwitchToBranch('wdio_configureoverrides')
    process.env.GIT_BRANCH = 'wdio_configureoverrides'
  })
  ...
  axeConfigurations.forEach((configObj) => {
    describe(configObj.description, () => {
      ...
      after(async () => {
        ...
        await verifyPagestateIssuesCount('configOverride', 'automation_WebdriverIO', currentBranch || undefined)
      });
